...
#     def get_most_recent_memories(self, number=50):
#         return self.memories[-number:]
...
#     def get_recency(self, id):
#         # Get recency score with an exponential decay function
#         # over the number of sandbox game hours since the memory was
#         # last retrieved. Our decay factor is 0.99.
#         memory = self.get_memory(id)
#         # Get the number of hours since the memory was last retrieved using datetime
#         current_time = datetime.datetime.strptime(self.sandbox.get_time(), "%Y-%m-%d %H:%M")
#         last_accessed_time = datetime.datetime.strptime(memory["last_accessed"], "%Y-%m-%d %H:%M")
#         hours_since_last_accessed = (current_time - last_accessed_time).total_seconds() / 3600
...
#             print(f"Relevance: {relevance}, Importance: {importance}, Recency: {recency}")
#             memory_scores[memory["id"]] = relevance + importance + recency
#             print(f"Score: {memory_scores[memory['id']]}")
        
#         # Sort the memories by their scores
#         sorted_memories = sorted(memory_scores.items(), key=lambda x: x[1], reverse=True)
#         # Return the top 3 memories
#         memories_to_return = [self.get_memory(memory[0]) for memory in sorted_memories[:2]]
#         # Update the last accessed time for each memory
#         for memory in memories_to_return:
#             self.access(memory["id"])
#         return memories_to_return
...
#     def reflect(self):
#         recent_memories = self.get_most_recent_memories()
#         memories_string = "\n".join([f"{memory['timestamp']}: {memory['content']}" for memory in recent_memories])
